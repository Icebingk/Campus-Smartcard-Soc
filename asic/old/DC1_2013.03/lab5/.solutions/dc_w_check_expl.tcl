...
# Compile the design. 
# Enable register replication using the -spg option
# Since the design is expected to have scan-chains inserted, include the "-scan" option.
...
# Verify the register replication naming style
printvar register_replication_naming_style
# Check to see if there are replicated registers
...
# Reset the cost priority to the default setting, where DRCs are prioritized
# over timing, and perform an incremental compile, to try to fix the DRC violation:
set_cost_priority -default
compile_ultra -spg -retime -scan -incremental

# The constraints report should confirm that there is no longer any DRC violation,
# and you may notice slight improvement in the WNS of the setup timing violations:
