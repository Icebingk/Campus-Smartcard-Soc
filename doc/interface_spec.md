...
### 3.1 AHB-Lite Master 接口（CPU 侧）

```verilog
// CPU → AHB Bus Matrix
output wire [31:0]  haddr;       // 地址
output wire [31:0]  hwdata;      // 写数据
output wire         hwrite;      // 读写控制
output wire [2:0]   hsize;       // 传输大小 (8/16/32-bit)
output wire [2:0]   hburst;      // 突发类型
output wire [1:0]   htrans;      // 传输类型 (IDLE/BUSY/NONSEQ/SEQ)
input  wire [31:0]  hrdata;      // 读数据
input  wire         hready;      // 就绪
input  wire         hresp;       // 响应 (OKAY/ERROR)
```
...
| 时钟名 | 频率 | 驱动域 | 说明 |
|--------|------|--------|------|
| `clk_sys` | 13.56 MHz | CPU + Bus + AES + EEPROM | 系统主时钟，射频场恢复或分频 |
